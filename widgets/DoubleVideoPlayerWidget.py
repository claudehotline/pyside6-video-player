...
from ui.double_video_player_widget import Ui_Form
...
class DoubleVideoPlayerWidget(QWidget):
    
    begin = Signal()
    
    def __init__(self, parent=None):
        super(DoubleVideoPlayerWidget, self).__init__(parent)
        self.ui = Ui_Form()
        self.ui.setupUi(self)
    

        self.label1 = self.ui.label1
        self.label2 = self.ui.label2
    ...
    def play(self):
        ...
        if play_status == False and play_is_setting_done:
            ...
            self.videoPlayer.get_video_frame_processor().result.connect(lambda result: self.show_image(result, self.label2))
            self.videoPlayer.get_video_frame_processor().frame.connect(lambda frame: self.show_image(frame, self.label1))
            self.begin.emit()
