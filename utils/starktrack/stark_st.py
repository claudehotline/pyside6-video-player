...
class STARK_ST_onnx(BaseTracker):
    def __init__(self, id):
        ...
        self.ort_sess_z = onnxruntime.InferenceSession(
            "model/tracking/stark/stark_st1_backbone_bottleneck_pe.onnx", providers=providers)
        self.ort_sess_x = onnxruntime.InferenceSession(
            "model/tracking/stark/stark_st1_complete.onnx", providers=providers)
...
class STARK_ST(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        network.load_state_dict(torch.load(
            self.params.checkpoint, map_location='cpu')['net'], strict=True)
        self.cfg = params.cfg
    ...
    def track(self, image, info: dict = None):
        ...
        if self.debug:
            x1, y1, w, h = self.state
            image_BGR = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
            cv2.rectangle(image_BGR, (int(x1), int(y1)), (int(
                x1+w), int(y1+h)), color=(0, 0, 255), thickness=2)
            save_path = os.path.join(self.save_dir, "%04d.jpg" % self.frame_id)
            cv2.imwrite(save_path, image_BGR)
        if self.save_all_boxes:
