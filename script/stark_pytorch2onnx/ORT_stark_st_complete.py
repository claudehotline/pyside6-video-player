...
class STARK(nn.Module):
    def __init__(self, backbone, bottleneck, position_embed, transformer, box_head, cls_head=None):
    ...
    def forward(self, img: torch.Tensor, mask: torch.Tensor, feat_vec_z_list: torch.Tensor,
                mask_vec_z_list: torch.Tensor, pos_vec_z_list: torch.Tensor):
        
        feat_vec_x, mask_vec_x, pos_vec_x = self.forward_backbone(img, mask)

        x_dict = {"feat": feat_vec_x, "mask": mask_vec_x, "pos": pos_vec_x}

        z_dict_list = []

        for i in range(self.num_extra_template):
            z_dict_list.append({"feat": feat_vec_z_list[i], "mask": mask_vec_z_list[i], "pos": pos_vec_z_list[i]})

        feat_dict_list = z_dict_list + [x_dict]

        seq_dict = merge_template_search(feat_dict_list)

        out_dict, _, _ = self.forward_transformer(seq_dict=seq_dict, run_box_head=True, run_cls_head=True)
...
if __name__ == "__main__":
    ...
    save_name = "stark_st1_complete.onnx"
    # update cfg
    args = parse_args()
    ...
    torch_model = STARK(backbone, bottleneck, position_embed, transformer, box_head, cls_head)
    print(torch_model)
    ...
    mask_vec_z_list = []
    pos_vec_z_list = []
    num_extra_template = 1
    for i in range(num_extra_template):
        feat_vec_z_list.append(to_numpy(feat_vec_z))
    feat_vec_z_list = np.array(feat_vec_z_list)
    feat_vec_z_list = torch.from_numpy(feat_vec_z_list)
    for i in range(num_extra_template):
        mask_vec_z_list.append(to_numpy(mask_vec_z))
    mask_vec_z_list = np.array(mask_vec_z_list)
    mask_vec_z_list = torch.from_numpy(mask_vec_z_list)
    for i in range(num_extra_template):
        pos_vec_z_list.append(to_numpy(pos_vec_z))
    pos_vec_z_list = np.array(pos_vec_z_list)
    pos_vec_z_list = torch.from_numpy(pos_vec_z_list)
    torch_outs = torch_model(img_x, mask_x, feat_vec_z_list, mask_vec_z_list, pos_vec_z_list)
    print('torch_outs[0]: ', torch_outs[0])
    print('torch_outs[1]: ', torch_outs[1])
    torch.onnx.export(torch_model,  # model being run
                      (img_x, mask_x, feat_vec_z_list, mask_vec_z_list, pos_vec_z_list),  # model input (a tuple for multiple inputs)
                      save_name,  # where to save the model (can be a file or file-like object)
                      export_params=True,  # store the trained parameter weights inside the model file
                      opset_version=11,  # the ONNX version to export the model to
                      do_constant_folding=False,  # whether to execute constant folding for optimization
                      input_names=['img_x', 'mask_x', 'feat_vec_z_list', 'mask_vec_z_list', 'pos_vec_z_list'],  # model's input names
                      output_names=['outputs_coord', 'outputs_score'],  # the model's output names
    ...
    for i in range(50):
        ...
        ort_inputs = {'img_x': to_numpy(img_x),
                      ...
                      'feat_vec_z_list': to_numpy(feat_vec_z_list),
                      'mask_vec_z_list': to_numpy(mask_vec_z_list),
                      'pos_vec_z_list': to_numpy(pos_vec_z_list)
                      }
        s_ort = time.time()
    ...
    for i in range(N):
        ...
        pos_vec_z_list = np.array(pos_vec_z_list)
        pos_vec_z_list = torch.from_numpy(pos_vec_z_list)
        # print("pos_vec_z_list: ", pos_vec_z_list.shape)
        # pytorch inference
        # img_x_cuda, mask_x_cuda, feat_vec_z_cuda, mask_vec_z_cuda, pos_vec_z_cuda = \
        #     img_x.cuda(), mask_x.cuda(), feat_vec_z.cuda(), mask_vec_z.cuda(), pos_vec_z.cuda()
        s_pyt = time.time()
